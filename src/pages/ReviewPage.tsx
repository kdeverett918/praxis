    ...
    <div className="mx-auto max-w-5xl pb-24 lg:pb-0">
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="font-display text-text-primary text-2xl md:text-3xl">Study Review</h1>
            <p className="font-body text-text-muted text-sm">{totalTopics} clinical cheat sheets</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8 flex flex-col gap-4 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          <Search className="text-text-muted absolute top-1/2 left-3.5 h-4 w-4 -translate-y-1/2" />
          <input
            type="text"
            placeholder="Search topics, Big Nine areas..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="border-border bg-surface font-body text-text-primary placeholder:text-text-muted/50 focus:border-primary focus:ring-primary w-full rounded-xl border py-2.5 pr-4 pl-10 text-sm focus:ring-1 focus:outline-none"
          />
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`font-body rounded-lg px-3 py-1.5 text-xs font-medium transition-all ${
              !activeFilter
      ...
      <div className="space-y-12">
        {filteredSections.map((section) => (
          <div key={section.key}>
            {/* Category Header */}
            <div
              className={`mb-6 rounded-2xl border ${section.borderColor} bg-gradient-to-r ${section.bgGradient} p-5`}
            >
              <div className="flex items-center gap-4">
                <div
                  className={`flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br ${section.gradient} shadow-lg`}
                >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${section.badgeColor}`}
                    >
                      {section.topics.length} topics
                    </span>
                  </div>
                  <h2 className="font-display text-text-primary mt-1 text-lg md:text-xl">
                    {section.label}
                  </h2>
                </div>
              </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <p className="font-body text-text-primary text-sm leading-snug font-semibold">
                          {topic.title}
                        </p>
                        <div className="mt-2 flex flex-wrap items-center gap-1.5">
                          <span
                            className={`inline-flex h-2 w-2 rounded-full ${section.dotColor}`}
                          />
                        ...
                        {topic.bigNine.length > 0 && (
                          <div className="mt-2.5 flex flex-wrap gap-1">
                            ...
                            {topic.bigNine.length > 3 && (
                              <span className="text-text-muted inline-flex rounded-md px-1.5 py-0.5 text-[10px] font-medium">
                                +{topic.bigNine.length - 3}
                              </span>
                            )}
                          </div>
                      ...
                      <ChevronRight className="text-text-muted mt-0.5 h-4 w-4 shrink-0 transition-transform group-hover:translate-x-1" />
                    </div>
                  </Card>
                </Link>
              ))}
      ...
      {filteredSections.length === 0 && (
        <div className="py-20 text-center">
          <BookOpen className="text-text-muted/30 mx-auto h-12 w-12" />
          <p className="font-body text-text-muted mt-4">No topics match your search.</p>
          <button
            onClick={() => {
              setSearch('')
              setActiveFilter(null)
            }}
            className="font-body text-primary hover:text-primary-hover mt-2 text-sm"
          >
            Clear filters
          </button>
        </div>
